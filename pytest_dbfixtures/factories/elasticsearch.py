...
def elasticsearch(process_fixture_name, hosts=None):
    """
    ...
    def elasticsearch_fixture(request):
        ...
        if not elasticsearch_hosts:
            elasticsearch_hosts = '%s:%s' % \
                                  (proc_fixture.host, proc_fixture.port)

        elasticsearch, _ = try_import('elasticsearch', request)

        client = elasticsearch.Elasticsearch(hosts=elasticsearch_hosts)
