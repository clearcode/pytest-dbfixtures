...
def get_port(ports):
    ...
    ports_set = parse_ports(ports)

    try:
        # We import `port_for` as late as it's possible
        # as we don't want to force library users to install this package
        # if it's not really needed.
        port_for = importlib.import_module('port_for')
    except ImportError:
        raise ImportError(
            'In order to use random port selection please'
            'install `port_for` package:\n'
            'pip install -U port_for')
